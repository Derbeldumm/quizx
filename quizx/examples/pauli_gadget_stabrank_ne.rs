...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 1..=nsamples {
        ...
        let mut renorm = Scalar4::one();
        let mut prob = Scalar4::one();
        let mut meas = vec![];
        ...
        for i in 0..qs {
            ...
            if p < 0.0 {
                println!("\nWARNING: p < 0 qubit {i}. p = {p}");
                p = 0.0;
            } else if p > 1.0 {
                println!("\nWARNING: p > 1 qubit {i}. p = {p}");
                p = 1.0;
            }
            ...
            let outcome = if rng.gen_bool(p) {
                ...
                // and <g|g> = <h'|h'>
                prob = renorm + Scalar4::minus_one() * prob;
                renorm = prob;
